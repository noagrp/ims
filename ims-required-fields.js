...
const REQUIRED_IDS=new Set([
  ...
  'bmAction','bmSource','bmDestination','bmReference','bmExistingGroup',
...
const REQUIRED_CLASSES=['resLookupValue','resQty','bmLookupValue','bmQty','scLookupValue'];
...
const BUTTON_REQUIREMENTS={
  ...
  scStart:['#scSrc','#scNo','#scDate','#scLookupValue'],
...
function syncConditional(){const mode=document.getElementById('scMode'),provider=document.getElementById('scProv');if(provider){if(mode?.value==='Sent To')requireEl(provider);else{provider.required=false;provider.removeAttribute('aria-required');}}
}
...
document.addEventListener('change',e=>{if(e.target?.id==='scMode')syncConditional();},true);
