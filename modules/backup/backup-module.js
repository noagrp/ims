...
function validateBackup(payload){
  if(!payload||Number(payload.imsBackupVersion)!==BACKUP_VERSION||!payload.collections||typeof payload.collections!=='object')throw new Error(`Unsupported IMS backup. Current backup version required: ${BACKUP_VERSION}.`);
...
async function restoreBackup(file){
  ...
  try{
    ...
    for(const name of names){for(const row of payload.collections[name]){
