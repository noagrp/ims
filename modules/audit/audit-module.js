...
// Optional consolidated Audit / Trace module.
// Superadmin-level visibility is inferred through centralized permissions;
// Manager-level visibility is Admin traces plus the current user's own traces.
// If unavailable, the legacy audit implementation remains as fallback.
...
async function reload(){
  ...
  if(can('users.role.edit')){
    ...
    const [adminSnap,ownSnap]=await Promise.all([
      ...
      getDocs(query(collection(db,'audit_traces'),where('performedBy','==',email)))
    ]);
    const map=new Map();
    adminSnap.forEach(d=>map.set(d.id,{id:d.id,...d.data()}));
    ownSnap.forEach(d=>map.set(d.id,{id:d.id,...d.data()}));
    records=[...map.values()].filter(x=>x.performedByRole==='admin'||x.performedBy===email);
  }
...
async function writeAudit(actionType,targetName,afterValue={}){
  ...
  await addDoc(collection(db,'audit_traces'),{
    ...
    targetName,
    targetId:'',
    summary:`${friendly(actionType)}: ${targetName}`,
    beforeValue:null,
    afterValue,
    changedFields:[],
    remark:'',
    metadata:{source:'audit-module'},
...
function setHeader(){
  ...
  document.querySelectorAll('.navBtn[data-tab]').forEach(btn=>{
    btn.className=`navBtn text-left px-3 py-2.5 rounded-xl text-xs font-semibold ${btn.dataset.tab==='audit'?'bg-red-600 text-white':'bg-slate-800/50 hover:bg-slate-800'}`;
  });
}

function roleOptions(){
  if(can('users.role.edit'))return ['admin','manager','superadmin'];
  return ['admin','self'];
}
...
function render(){
  ...
  mount.innerHTML=`<div id="imsAuditModule" class="space-y-5">
    ${card('Audit Filters',`<div class="grid sm:grid-cols-2 lg:grid-cols-5 gap-2">
      ${label('Search',`<input id="imsAuditSearch" class="${inputCls}" placeholder="User, item, action, remark...">`)}
      ${label('Role',`<select id="imsAuditRole" class="${inputCls}"><option value="">All Visible</option>${roles.map(r=>`<option value="${r}">${r==='self'?'My Actions':r[0].toUpperCase()+r.slice(1)}</option>`).join('')}</select>`)}
      ${label('Module',`<select id="imsAuditModuleFilter" class="${inputCls}"><option value="">All Modules</option>${modules.map(x=>`<option>${esc(x)}</option>`).join('')}</select>`)}
      ${label('From Date',`<input type="date" id="imsAuditFrom" class="${inputCls}">`)}
      ${label('To Date',`<input type="date" id="imsAuditTo" class="${inputCls}">`)}
    </div>${exportButtons}`)}
    ${card('System Audit',`<div id="imsAuditCount" class="text-xs text-slate-500 mb-2"></div><div class="overflow-auto max-h-[68vh] border border-slate-800 rounded-xl"><table class="w-full min-w-[1250px] text-xs"><thead class="sticky top-0 bg-slate-950 text-slate-400"><tr><th class="p-2"><input id="imsAuditSelectAll" type="checkbox"></th>${[['performedAt','Date'],['performedBy','User'],['performedByRole','Role'],['module','Module'],['actionType','Action'],['targetName','Target'],['changedFields','Changed'],['remark','Remark']].map(([f,n])=>`<th class="p-2 text-left cursor-pointer imsAuditSort" data-field="${f}">${n} ↕</th>`).join('')}<th class="p-2">Details</th></tr></thead><tbody id="imsAuditBody"></tbody></table></div>`) }
  </div>`;

  ['imsAuditSearch','imsAuditRole','imsAuditModuleFilter','imsAuditFrom','imsAuditTo'].forEach(id=>{
    const el=byId(id); if(el)el.oninput=applyFilters;
  });
...
function applyFilters(){
  const q=norm(byId('imsAuditSearch')?.value||'');
  const role=byId('imsAuditRole')?.value||'';
  const module=byId('imsAuditModuleFilter')?.value||'';
  const from=byId('imsAuditFrom')?.value||'';
  const to=byId('imsAuditTo')?.value||'';
  const email=auth.currentUser?.email||'';
  filtered=records.filter(x=>{
    const d=String(x.performedAt||'').slice(0,10);
    const roleOk=!role||(role==='self'?x.performedBy===email:x.performedByRole===role);
    return(!q||norm(JSON.stringify(x)).includes(q))&&roleOk&&(!module||x.module===module)&&(!from||d>=from)&&(!to||d<=to);
  });
  filtered.sort((a,b)=>{
    const av=Array.isArray(a[sort.field])?a[sort.field].join(', '):(a[sort.field]??'');
    const bv=Array.isArray(b[sort.field])?b[sort.field].join(', '):(b[sort.field]??'');
...
function showDetail(id){
  const x=records.find(y=>y.id===id);
  if(!x)return;
  alert(`${x.summary||friendly(x.actionType)}\n\nBefore:\n${JSON.stringify(x.beforeValue,null,2)}\n\nAfter:\n${JSON.stringify(x.afterValue,null,2)}\n\nRemark: ${x.remark||'—'}\nBy: ${x.performedBy} (${x.performedByRole})\nAt: ${x.performedAt}`);
}

async function exportAudit(selected){
  if(!can('audit.export.csv'))return;
  let rows=filtered;
  if(selected){
    const ids=[...document.querySelectorAll('.imsAuditRowCheck:checked')].map(x=>x.value);
    rows=filtered.filter(x=>ids.includes(x.id));
    if(!rows.length){alert('Select audit rows first.');return;}
  }
  const out=rows.map(x=>({Date:x.performedAt,User:x.performedBy,Role:x.performedByRole,Module:x.module,Action:friendly(x.actionType),Target:x.targetName,Changed:(x.changedFields||[]).join(', '),Before:JSON.stringify(x.beforeValue),After:JSON.stringify(x.afterValue),Remark:x.remark||''}));
  const wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(out),'Audit');
  XLSX.writeFile(wb,`IMS_Audit_${selected?'Selected':'Filtered'}_${nowISO().slice(0,10)}.xlsx`);
  await writeAudit('EXPORT_AUDIT',selected?'Selected Audit Export':'Filtered Audit Export',{count:rows.length});
}

function interceptNavigation(){
  document.addEventListener('click',async event=>{
    const btn=event.target.closest?.('.navBtn[data-tab="audit"]');
    if(!btn||!can('audit.view'))return;
    ...
    try{
      ...
      console.warn('IMS audit module failed; legacy fallback remains available.',error);
    }
  },true);
}
