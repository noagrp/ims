...
// Consolidated IMS runtime. Legacy business files are not loaded. Modules boot
// only after the authenticated shell exists, and every business import must
// publish its expected IMS ownership global before it is reported as loaded.
...
function bindDirectNavigation(){
  const stock=document.querySelector('.navBtn[data-tab="stock"]');if(stock&&window.IMSInventory)stock.onclick=()=>window.IMSInventory.show('overview');
  const workspace=document.querySelector('.navBtn[data-tab="workspace"]');if(workspace&&window.IMSWorkspace)workspace.onclick=()=>window.IMSWorkspace.show();
}

function publishStatus(loaded,failed,skipped){
  const owners=Object.fromEntries(MODULES.filter(x=>x.owner).map(x=>[x.id,{owner:x.owner,ready:Boolean(window[x.owner]),allowed:allowed(x)}]));
  window.IMSModules=Object.freeze({loaded:[...loaded],failed:[...failed],skipped:[...skipped],owners,registry:MODULES,legacyFallbacks:false});
  window.dispatchEvent(new CustomEvent('ims:modules-ready',{detail:window.IMSModules}));
  console.info('IMS consolidated module status',window.IMSModules);
}
...
async function bootOptionalModules(){
  ...
  publishStatus(loaded,failed,skipped);
  if(window.IMSWorkspace&&!document.querySelector('[data-ims-workspace-module="1"]'))window.IMSWorkspace.show();
  return window.IMSModules;
}
