import { can, currentRole } from './ims-permissions.js?v=20260910-20';

const IMS_BUILD='20260910-20';
...
{id:'registration-excel',src:'./modules/registration/registration-excel.js',roles:['manager','superadmin'],owner:'IMSRegistrationExcel'},
...
function refreshImportExport(){if(window.IMSRegistrationCSV?.install){queueMicrotask(()=>window.IMSRegistrationCSV?.install?.());setTimeout(()=>window.IMSRegistrationCSV?.install?.(),100);}if(window.IMSRegistrationExcel?.install){queueMicrotask(()=>window.IMSRegistrationExcel?.install?.());setTimeout(()=>window.IMSRegistrationExcel?.install?.(),150);setTimeout(()=>window.IMSRegistrationExcel?.install?.(),450);}}
